...
var RRCS_Server = function RRCS_Server(local, remote, cb) {
  ...
  server.on('NotFound', function (method, params, callback) {
    cb.warn("".concat(method, " not found"), params);
  });
  /**
  ...
  */
  ...
  server.on('SendStringOff', function (err, params, callback) {
    if (err) cb.error('sendStringOff error', err);else cb.log('sendStringOff', params);
    callback(null, params[0]);
  });
  /**
   * sendString notification
  ...
  server.on('LogicSourceChange', function (err, params, callback) {
    if (err) cb.error('logicSourceChange error', err);else cb.log('logicSourceChange', params);
    callback(null, params[0]);
  });
  /** LOG ALARM EVENTS **/

  server.on('ConfigurationChange', function (err, params, callback) {
    if (err) cb.error('configurationChange error', err);else cb.log('configurationChange');
  });
  server.on('UpstreamFailed', function (err, params, callback) {
    if (err) cb.error('upstreamFailed error', err);else cb.warn('upstreamFailed', params);
    callback(null, params[0]);
  });
  server.on('UpstreamFailedCleared', function (err, params, callback) {
    if (err) cb.error('upstreamFailedCleared error', err);else cb.log('upstreamFailedCleared', params);
    callback(null, params[0]);
  });
  server.on('DownstreamFailed', function (err, params, callback) {
    if (err) cb.error('downstreamFailed error', err);else cb.warn('downstreamFailed', params);
    callback(null, params[0]);
  });
  server.on('DownstreamFailedCleared', function (err, params, callback) {
    if (err) cb.error('downstreamFailedCleared error', err);else cb.log('downstreamFailedCleared', params);
    callback(null, params[0]);
  });
  server.on('NodeControllerFailed', function (err, params, callback) {
    if (err) cb.error('nodeControllerFailed error', err);else cb.warn('nodeControllerFailed', params);
    callback(null, params[0]);
  });
  server.on('NodeControllerReboot', function (err, params, callback) {
    if (err) cb.error('nodeControllerReboot error', err);else cb.warn('nodeControllerReboot', params);
    callback(null, params[0]);
  });
  server.on('ClientFailed', function (err, params, callback) {
    if (err) cb.error('clientFailed error', err);else cb.warn('clientFailed', params);
    callback(null, params[0]);
  });
  server.on('ClientFailedCleared', function (err, params, callback) {
    if (err) cb.error('clientFailedCleared error', err);else cb.log('clientFailedCleared', params);
    callback(null, params[0]);
  });
  server.on('PortInactive', function (err, params, callback) {
    if (err) cb.error('portInactive error', err);else cb.log('portInactive', params);
    callback(null, params[0]);
  });
  server.on('PortActive', function (err, params, callback) {
    if (err) cb.error('portActive error', err);else cb.log('portActive', params);
    callback(null, params[0]);
  });
